...
var _ = Describe("DynamicResourceRequestController", func() {
    ...
    When("resource triggers deletion via label", func() {
        ...
        It("triggers delete workflows when the label is added", func() {
            By("ensuring the resource status shows delete workflows triggered")
            Expect(fakeK8sClient.Get(ctx, resReqNameNamespace, resReq)).To(Succeed())
            message := resourceutil.GetStatus(resReq, "message")
            Expect(message).To(Equal("Delete Workflows Triggered"))
